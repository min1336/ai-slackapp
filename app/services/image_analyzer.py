...
1. 문서에서 읽을 수 있는 모든 정보를 key-value 형태로 추출하세요.
   ...
   년도가 불명확하면 오늘 날짜({today}) 기준 과거 방향으로 추론하세요.
   결항은 항상 과거 사건이므로 오늘 이전의 가장 가까운 날짜를 선택합니다.
   예: 오늘이 2026-03-20이고 "18MAR"이면 → 2026-03-18.
   예: 오늘이 2026-01-05이고 "18MAR"이면 → 2025-03-18.
2. 문서의 유효성을 검증하세요. 다음 중 해당하는 문제가 있으면 rejection_reasons에 추가:
   - 이미지가 흐리거나 텍스트를 읽을 수 없는 경우
   - 결항이 아닌 지연(delay) 안내 문서인 경우
   - 고객명이 누락된 경우 (어떤 형태든 사람 이름이 보이면 누락이 아님.
     메신저 대화, 앱 스크린샷, 이메일 등에서 이름이 언급되면 고객명으로 인정.
     이름이 문서 어디에도 전혀 보이지 않는 경우에만 누락으로 판단)
   - 날짜가 완전히 누락된 경우 (월/일이라도 읽을 수 있으면 누락이 아님)
3. 추출한 정보를 바탕으로 문서 내용을 1~2문장 한국어로 요약하세요.
